...
public class GettingJuwel_UI : MonoBehaviour
{
    ...
    void Update()
    {
        for (int i = 0; i < 5; i++)
        {
            if (GameSystem.GetJuwelGetted(i) || GameSystem.GetJuwelCollection(i, GameSystem.WasPlayStage) == true)
            {
                SetJuwelGettingUI(i, true);
            }
            else
            {
                SetJuwelGettingUI(i, false);
            }
        }
    }

    void SetJuwelGettingUI(int juwelNum, bool gettedTrue)
    {
        Color color;
